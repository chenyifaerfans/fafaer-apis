...
def validate_fields(field, upload_max_size=0, upload_types=[]):
    file_name = getattr(field, 'name')
    names = file_name.rsplit(".", 1)
    if len(names) != 2:
        raise serializers.ValidationError(_("文件名错误"), code=_("%s_invalid" % file_name))
...
def clean_fields(field, upload_max_size=0, upload_types=[]):
    ...
    names = file_name.rsplit(".", 1)
    if len(names) != 2:
        raise forms.ValidationError(u"文件名错误", code=_("%s_invalid" % file_name))
